...
- **Summaries / Sumários:** Teacher view with lesson list showing summary and attention box for each lesson. Clicking a lesson expands to show full details or launches student presentation mode (full-screen modal with only lesson number, date, summary, and attention box). Includes year/group/date filtering and launchpad for **Daily Self-Assessment / Autoavaliação Diária** per lesson.
